...
def table_for_of(simulation, period=None, check_validity=False, save_tables=False):
    ...
    table['ind'] = table['ind'].rename(
        columns={'men': 'idmen', 'idfoy': 'idfoy', 'id': 'noi', 'statmarit': 'civilstate'})
...
#        for name, groupfor nom in ('menage','declar','fam'):
# for nom in ('menage','declar','fam'): in trav.groupby([ident,'period']):
...
#    print "le temps passé à s'occuper des qui a été",time_qui
    ...
    for ent in ['men', 'foy']:
        ...
        if ent == 'men':
            ...
            table[ent]['nbinde'] = (
                (table[ent]['nb_persons'] - 1) * (table[ent]['nb_persons'] - 1 <= 5)
                ) + 5 * (table[ent]['nb_persons'] - 1 > 5)
            table['fam'] = qui0
    ...
    table['idfoy'] = table['idfoy'][table['idfoy']['idfoy'] >= 10]
